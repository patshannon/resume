...
export default function SkillsRadarChart() {
  ...
  useEffect(() => {
    // Apply gradients to Hard Skills chart - black/white/grey theme
    if (hardChartRef.current) {
      const ctx = hardChartRef.current.ctx;
      const gradient = createGradient(ctx, 'rgba(255, 255, 255, 0.15)', 'rgba(255, 255, 255, 0.05)');
      hardChartRef.current.data.datasets[0].backgroundColor = gradient;
      hardChartRef.current.data.datasets[0].borderColor = 'rgba(255, 255, 255, 0.7)';
      hardChartRef.current.data.datasets[0].pointBackgroundColor = '#ffffff';
      hardChartRef.current.data.datasets[0].pointBorderColor = '#000000';
      hardChartRef.current.data.datasets[0].pointHoverBackgroundColor = '#000000';
      hardChartRef.current.data.datasets[0].pointHoverBorderColor = '#ffffff';
      hardChartRef.current.update();
    }

    // Apply gradients to Soft Skills chart - black/white/grey theme
    if (softChartRef.current) {
      const ctx = softChartRef.current.ctx;
      const gradient = createGradient(ctx, 'rgba(156, 163, 175, 0.2)', 'rgba(156, 163, 175, 0.05)');
      softChartRef.current.data.datasets[0].backgroundColor = gradient;
      softChartRef.current.data.datasets[0].borderColor = 'rgba(156, 163, 175, 0.8)';
      softChartRef.current.data.datasets[0].pointBackgroundColor = '#9ca3af';
      softChartRef.current.data.datasets[0].pointBorderColor = '#000000';
      softChartRef.current.data.datasets[0].pointHoverBackgroundColor = '#000000';
      softChartRef.current.data.datasets[0].pointHoverBorderColor = '#9ca3af';
    ...
    <div className="w-full flex flex-col lg:flex-row gap-16 lg:gap-12 items-center justify-center px-4">
      ...
      >
        <div className="relative">
          ...
          <div className="bg-black/40 backdrop-blur-sm rounded-2xl p-4 sm:p-8 border border-white/20 shadow-xl hover:border-white/30 transition-all duration-500">
      ...
      >
        <div className="relative">
          ...
          <div className="bg-black/40 backdrop-blur-sm rounded-2xl p-4 sm:p-8 border border-gray-500/30 shadow-xl hover:border-gray-400/40 transition-all duration-500">
